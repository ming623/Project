...
#version3 --> read processed combined inputs from proteome hits and tblastn hits

#df1 is the bltnm8 table
#df1 <- read.table("GST_bltnm8",sep="\t")

getregion <- function(speciesdf) {
#sourcedf is extracted dataframe of col2 = chromosome, col9 = start, col10 = end
sourcedf <- read.table(speciesdf,sep="\t")
...
for (i in 1:length(chr.uniq)) {
  #extract row(s) that corresponds each chromosome in sourcedf to df3
  each.chr <- sourcedf[,1] == chr.uniq[i]
  df3 <- sourcedf[each.chr,]
...
write.table(df4,file="chrregion",sep = "\t")
}
